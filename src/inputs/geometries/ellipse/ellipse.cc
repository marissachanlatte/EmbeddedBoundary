...
double EllipseGeometry::BoundaryDerivatives(helpers::Point a_point,
  ...
  if (degree[0] == 0 && degree[1] == 0){
    return 2*std::pow(a_point.value(1), 2) + std::pow(a_point.value(0), 2) - 1;
  }

  else if (degree[0] == 1 && degree[1] == 0){ 
    return 2*a_point.value(0);
  }

  else if (degree[0] == 0 && degree[1] == 1){
    return 4*a_point.value(1);
  }
...
int EllipseGeometry::Inside(std::array<double, 2> point){
  if ((std::pow(point[0], 2) + 2*std::pow(point[1], 2)) > 1){
    return 0;
  }
  else if ((std::pow(point[0], 2) + 2*std::pow(point[1], 2)) < 1){
    return 1;
  }
  else {
    return 2;
